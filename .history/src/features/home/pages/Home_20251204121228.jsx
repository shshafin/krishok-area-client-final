...
export default function Home() {
  ...
  const [selectedPost, setSelectedPost] = useState(null); // for notification modal
  const [modalOpen, setModalOpen] = useState(false);

  const location = useLocation();
  const postIdFromQuery = new URLSearchParams(location.search).get("postId");
  ...
  // 🔹 Open modal if notification query param exists
  useEffect(() => {
    if (postIdFromQuery) {
      // ideally, fetch post data from InfiniteFeed store or API
      setSelectedPost({ _id: postIdFromQuery }); // minimum post object
      setModalOpen(true);
    }
  }, [postIdFromQuery]);
  ...
  return (
    <>
      ...
      {modalOpen && selectedPost && (
        <PostComposerModal
          open={modalOpen}
          mode="view" // "view" mode for post modal
          post={selectedPost} // pass selected post object

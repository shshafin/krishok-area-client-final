...
export default function InfiniteFeed() {
  ...
  useEffect(() => {
    ...
    // We wait for currentUserId to be loaded to properly adapt the post
    if (!queryPostId || !currentUserId) return;

    // Prevent re-running if we are already viewing this post
    if (sameId(activePostId, queryPostId)) return;

    const checkAndOpenPost = async () => {
      // 2. Check if post is already in the feed
      ...
      if (existingPost) {
        ...
        // 3. Post not found in feed (old post), fetch it manually
        try {
          ...
          console.error("Failed to fetch notification post", error);
          toast.error("পোস্টটি পাওয়া যায়নি বা মুছে ফেলা হয়েছে");
    ...
    checkAndOpenPost();
  }, [searchParams, currentUserId, posts, activePostId, setSearchParams]);
  // =========================================================
  // CHANGED: END
  // =========================================================

...
const adaptFeedPost = (rawPost, viewerId) => {
  ...
  // Helper to handle string/object urls
  const getSrc = (item) => {
    if (!item) return null;
  ...
  // 1. COLLECT ALL MEDIA ITEMS
  const combinedGallery = [];

  // A. Handle Videos (Priority 1)
  // Check the 'videos' array from your console log
  const rawVideos = Array.isArray(rawPost.videos) ? rawPost.videos : [];
  // Also check single 'video' field just in case
  if (rawPost.video) rawVideos.push(rawPost.video);
  if (rawPost.media?.video) rawVideos.push(rawPost.media.video);

  // Take the first valid video found
  for (const v of rawVideos) {
    const src = ensureAbsoluteUrl(getSrc(v));
    if (src) {
      combinedGallery.push({ type: "video", src });
      break; // Usually we only show 1 main video in feed, remove break if you want multiple
    }
  }

  // B. Handle Images (Priority 2)
  const rawImages = Array.isArray(rawPost.images) ? rawPost.images : [];
  // Also check legacy fields
  if (rawPost.image) rawImages.push(rawPost.image);
  if (rawPost.mediaUrl) rawImages.push(rawPost.mediaUrl);
  if (rawPost.coverPhoto) rawImages.push(rawPost.coverPhoto);
  if (rawPost.media?.images && Array.isArray(rawPost.media.images)) {
    rawImages.push(...rawPost.media.images);
  }
  ...
  // 2. ASSIGN MAIN MEDIA
  // 'media' is usually the cover item (Video or 1st Image)
  // 'mediaGallery' is the list of all items (for the slider)
  const media = combinedGallery.length > 0 ? combinedGallery[0] : null;
  const mediaGallery = combinedGallery;

  // 3. Likes & Comments Logic
  ...
  return {
    ...
    media,
    mediaGallery, // This now contains [{type:'video'...}, {type:'image'...}]

  ...
  const handleAddComment = useCallback(
    async (postId, text) => {
      ...
      try {
        ...
        // 1. Extract the actual comment data from response
        // Backend might return { data: comment } or { comment: ... } or just comment
        const payload = response?.data ?? response;
        const actualCommentData = payload?.comment ?? payload?.data ?? payload;

        // 2. Get the REAL ID from backend
        // If this resolves to null, the delete will fail until refresh
        const realId = resolveId(actualCommentData);
        const finalId = realId ?? `comment-${Date.now()}`;

        // 3. Create the User object for the comment
        ...
        const newComment = {
          id: finalId, // Used for React Keys
          _id: finalId, // <--- CRITICAL: Used for Deletion filtering

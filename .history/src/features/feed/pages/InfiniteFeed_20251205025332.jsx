...
const adaptFeedPost = (rawPost, viewerId) => {
  ...
  // 3. Create the Gallery (Images initially)
  ...
  // --- CHANGED LOGIC START ---
  // If there is a video, add it to 'media' AND add it to 'mediaGallery'
  if (firstVideoCandidate) {
    const videoSrc = ensureAbsoluteUrl(firstVideoCandidate);
    if (videoSrc) {
      const videoObj = { type: "video", src: videoSrc };

      // Set the main media
      media = videoObj;

      // Add video to the START of the gallery so it appears with images
      mediaGallery.unshift(videoObj);
    }
  }

  // If no main media yet, pick the first from gallery (which could be an image)
  ...
  return {
    ...
    media,
    mediaGallery, // Now contains [Video, Image, Image...]

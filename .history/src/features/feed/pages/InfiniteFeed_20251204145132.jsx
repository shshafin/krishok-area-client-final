...
export default function InfiniteFeed() {
  ...
  // NEW: Ref to track the URL history for smooth closing
  ...
  // =========================================================
  // CHANGED: ROBUST URL LOGIC
  // Handles opening via URL and Closing when URL is removed
  // =========================================================
  useEffect(() => {
    const queryPostId = searchParams.get("postId");

    // Case 1: URL has ID (Notification Open)
    if (queryPostId) {
      // Remember we saw this ID
    ...
    // Case 2: URL has NO ID, but we just came from a URL (Notification Close)
    else if (previousUrlPostId.current) {
      // We just navigated away from a post URL, so we must close the modal.
      setActivePostId(null);
      setActiveModalMode("comments");
      setDeletingCommentId(null);
      // Reset the ref so this doesn't run for manual clicks
  ...
  const closeModal = useCallback(() => {
    ...
    if (urlPostId) {
      // 1. If opened via URL (Notification):
      // ONLY remove the URL param. The useEffect (Case 2) will detect this
      // change and close the modal state automatically.
      const newParams = new URLSearchParams(searchParams);
      newParams.delete("postId");
      navigate(`${location.pathname}?${newParams.toString()}`, {
        replace: true,
      ...
      // 2. If opened Manually (Feed):
      // Just close the state directly.
      setActivePostId(null);
      setActiveModalMode("comments");
      setDeletingCommentId(null);
    }
  ...
  const handleAddComment = useCallback(
    async (postId, text) => {
      ...
      try {
        ...
        // 🔍 DEBUG: Check your console to see exactly what the backend sends back
        console.log("SERVER RESPONSE FOR COMMENT:", response);

        // 1. Normalize the response structure
        // Sometimes axios returns data inside .data, sometimes the API wrapper does it
        let payload = response?.data ?? response;

        // 2. Find the comment object
        // It might be in payload.comment, payload.data, or payload itself
        let actualCommentData = payload?.comment ?? payload?.data ?? payload;

        // SPECIAL CHECK: Did the backend return the WHOLE updated post?
        // If so, the new comment is likely the last one in the comments array
        if (
          actualCommentData?.comments &&
          Array.isArray(actualCommentData.comments)
        ) {
          const commentsArray = actualCommentData.comments;
        ...
        if (!realId) {
          console.error("COULD NOT FIND ID IN:", actualCommentData);
        }

        // If we still don't have an ID, we use the fallback (which will fail to delete until refresh)
        ...
        const newComment = {
          id: finalId,
          _id: finalId, // Critical for deletion
    ...
    try {
      ...
      setPosts((prev) =>
        prev.map((post) => {
          ...
          // We check both _id and id to be safe
  ...
  const handleDeletePost = useCallback(
    (postId) => {
      ...
      if (sameId(activePostId, postId)) {
        setActivePostId(null);
        // Also clean URL if we are deleting the active post
        const newParams = new URLSearchParams(searchParams);

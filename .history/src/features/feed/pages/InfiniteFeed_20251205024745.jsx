  ...
  const handleAddComment = useCallback(
    async (postId, text) => {
      ...
      try {
        ...
        // 2. EXTRACTION LOGIC BASED ON YOUR DEBUG OUTPUT
        // The server returns { success: true, post: { comments: [...] } }
        if (payload?.post?.comments && Array.isArray(payload.post.comments)) {
          const commentsArray = payload.post.comments;
          // The new comment is always the last one in the array
          actualCommentData = commentsArray[commentsArray.length - 1];
        }
        // Fallback: checks if server returns just { comment: ... }
        ...
        // Safety check to ensure we got a real Mongo ID
        const finalId = realId ?? `comment-${Date.now()}`;

        // 4. Construct the Comment Object for Frontend State
        ...
        const newComment = {
          id: finalId,
          _id: finalId, // CRITICAL: This must be the real Mongo ID
        ...
        setPosts((prev) =>
          prev.map((post) => {
            ...
            const updatedRaw = post.raw
              ? {
                  ...post.raw,
                  // Update raw data with the extracted comment
                  comments: [
                    ...(post.raw.comments ?? []),

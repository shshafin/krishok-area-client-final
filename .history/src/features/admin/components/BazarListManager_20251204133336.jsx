...
export default function BazarListManager({
  ...
  onDeleteConfirmed = async () => {}, // <--- pass backend delete function from parent
}) {
  ...
  const deleteEntryAnimation = (entry) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      // ✅ Call backend delete function passed from parent
      await onDeleteConfirmed(confirmEntry);
      // Animate removal
      deleteEntryAnimation(confirmEntry);
      toast.success(deleteToastMessage(confirmEntry));
    } catch (err) {
      console.error(err);
      toast.error("Failed to delete entry from server");
    } finally {

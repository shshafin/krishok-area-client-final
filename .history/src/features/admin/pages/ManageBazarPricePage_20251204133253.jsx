...
export default function ManageBazarPricePage() {
  ...
  const handleDeleteConfirmed = async (entry) => {
    try {
      ...
      if (!res?.success) {
        throw new Error(res?.message || "Failed to delete entry");
      }
      // Remove from frontend state handled inside BazarListManager
    } catch (err) {
      console.error(err);
      toast.error(err.message || "Server error while deleting entry");
  ...
  return (
    <>
      ...
      <BazarListManager
        ...
        onDeleteConfirmed={handleDeleteConfirmed} // ✅ Pass API call here
      />
    </>
  );
}

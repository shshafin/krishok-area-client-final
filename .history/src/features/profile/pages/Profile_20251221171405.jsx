...
// --- FIX 2: Robust Media Normalizer (Same logic as Home Page) ---
const normalizeMedia = (post) => {
  const images = [];
  const videos = [];

  // 1. Collect Videos from all possible fields
  ...
  // 2. Collect Images from all possible fields
  ...
  rawImages.forEach((img) => {
    // Handle if image is object or string
    const srcRaw =
  ...
  useEffect(() => {
    const loadCurrentUserAndProfile = async () => {
      try {
        ...
        // Handle if response is array or object with .posts
        const rawPostsArray = Array.isArray(postsResponse)
          ? postsResponse
          : postsResponse?.posts || [];

        const normalizedPosts = rawPostsArray.map((post) => {
          // --- FIX 3: Pass the WHOLE post object to normalizer ---

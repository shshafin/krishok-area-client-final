...
// --- FIX: Smart URL Helper (Prevents double http:// issues) ---
const ensureAbsoluteUrl = (url) => {
  if (!url) return null;
  // If it's already an absolute URL or blob, return as is
  if (url.startsWith("http") || url.startsWith("blob:")) return url;
  // Otherwise, prepend the baseApi
  return `${baseApi}${url}`;
};

// --- FIX: Robust Media Normalizer ---
const normalizeMedia = (post) => {
  const images = [];
  const videos = [];

  // 1. Check for Videos (array or singular)
  ...
  rawVideos.forEach((vid) => {
    ...
    const src = typeof vid === "object" ? vid.url || vid.src || vid.path : vid;
    const cleanSrc = ensureAbsoluteUrl(src);
    if (cleanSrc) videos.push({ type: "video", src: cleanSrc });
  });

  // 2. Check for Images (array, singular, mediaUrl, coverPhoto)
  ...
  rawImages.forEach((img) => {
    const src = typeof img === "object" ? img.url || img.src || img.path : img;
    const cleanSrc = ensureAbsoluteUrl(src);
    if (cleanSrc) images.push({ type: "image", src: cleanSrc });
  });
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const loadCurrentUserAndProfile = async () => {
      try {
        ...
        // --- FIX: Handle API response safely (Array vs Object) ---
        const rawPostsArray = Array.isArray(postsResponse)
          ? postsResponse
          : postsResponse?.posts || [];

        const normalizedPosts = rawPostsArray.map((post) => {
          // --- FIX: Use robust media normalizer ---
          const { media, mediaGallery } = normalizeMedia(post);
          ...
          return {
            ...
            author: {
              ...
              avatar: post.user?.profileImage
                ? ensureAbsoluteUrl(post.user.profileImage)
  ...
  const submitComposer = async (payload) => {
    ...
    try {
      ...
      const { media, mediaGallery } = normalizeMedia(postData);

...
const ensureAbsoluteUrl = (url) => {
  ...
  // Handle relative paths from backend (e.g. "/uploads/image.png")
...
export default function ProfilePage() {
    ...
    const loadCurrentUserAndProfile = async () => {
      try {
        ...
        const normalizedPosts = rawPostsArray.map((post) => {
          ...
          // Prioritize 'name' > 'fullName' > 'username'
          ...
          return {
            ...
            // --- CRITICAL FIX: Ensure mediaGallery exists ---
            mediaGallery: gallery,
            media: gallery.length > 0 ? gallery[0] : null,

            author: {
              id: postUser._id || post.userId,
              name: authorName, // Using the fixed name logic
  ...
  const submitComposer = async (payload) => {
    ...
    try {
      ...
      // Re-build gallery for new post logic
      const gallery = [];
      if (postData.images)

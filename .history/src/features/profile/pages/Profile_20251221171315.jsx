...
const normalizeMedia = (post) => {
  ...
  return {
    ...
    // Return sanitized arrays too, just in case
    cleanImages: images,
    cleanVideos: videos,
  };
};
    ...
    const loadCurrentUserAndProfile = async () => {
      try {
        ...
        const normalizedPosts = rawPostsArray.map((post) => {
          // --- FIX: Normalize Media ---
          const { media, mediaGallery, cleanImages, cleanVideos } =
            normalizeMedia(post);

          // --- FIX: Name Logic ---
          // Check 'name' -> 'fullName' -> 'username'
          const authorName =
            post.user?.name ||
            post.user?.fullName ||
            post.user?.username ||
          ...
          return {
            ...
            // Explicitly set media properties
            media: media,
            mediaGallery: mediaGallery, // This MUST be here for PostCard
            images: cleanImages, // Overwrite with absolute URLs
            videos: cleanVideos, // Overwrite with absolute URLs

            author: {
              id: post.user?._id || post.userId,
              name: authorName, // Use the priority name

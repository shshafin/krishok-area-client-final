...
export default function ProfilePage() {
  ...
  const openPostComments = useCallback((postId, index = 0) => {
    if (!postId) {
      console.error("Cannot open post: ID is missing");
      return;
    }
    console.log("OPEN POST TRIGGERED:", postId);
    setActivePostMode("comments");
  ...
  useEffect(() => {
    const loadCurrentUserAndProfile = async () => {
      try {
        ...
        // --- FIX STARTS HERE: ROBUST DATA NORMALIZATION ---
        const normalizedPosts = rawPostsArray.map((post) => {
          // 1. FIX ID: Ensure we get a valid ID (check _id and id)
          const realId = post._id || post.id;

          // 2. FIX USER: Check both 'user' and 'author' keys
          const postUser = post.user || post.author || {};

          // 3. FIX NAME: Ensure we prioritize the display name
          const realName =
          ...
          return {
            ...post,
            id: realId, // <--- IMPORTANT: Using the fixed ID
            mediaGallery: gallery,
            media: gallery.length > 0 ? gallery[0] : null,
            author: {
              id: postUser._id || postUser.id || post.userId,
              name: realName,
            ...
            comments: (post.comments || []).map((c) => ({
              id: c._id || c.id, // Ensure comment ID is also safe
              text: c.text,
              author: {
  ...
  const toggleLike = async (postId) => {
    if (!postId) {
      console.error("toggleLike failed: No Post ID");
      return;
    }
  ...
  const submitComposer = async (payload) => {
    ...
    try {
      ...
      const newPost = {
        ...postData,
        id: postData._id || postData.id, // Ensure ID on create too
        mediaGallery: gallery,

...
// Note: We are overriding CSS with inline styles, so this import is less critical now
...
export default function ProfilePage() {
  ...
  const openPostComments = useCallback((postId, index = 0) => {
    console.log("OPEN POST TRIGGERED:", postId); // Debug Log
    setActivePostMode("comments");
  ...
  const toggleLike = async (postId) => {
    console.log("LIKE CLICKED:", postId); // Debug
  ...
  return (
    <div className="profile-page">
      ...
      {/* --- INLINE CSS LAYOUT FIX --- */}
      {/* display: flex forces elements side-by-side. No overlap possible. */}
      <div
        style={{
          display: "flex",
          flexDirection: "row",
        ...
        {/* FEED: Takes remaining space + Forced Clickability */}
        <section
          className="post-feed"
          style={{
            flex: 1,
            ...
            position: "relative",
            zIndex: 10,
            pointerEvents: "auto", // <--- FORCE CLICKS
          }}>
          {isOwner && (
            <CreatePost

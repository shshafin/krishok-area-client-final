...
const ensureAbsoluteUrl = (url) => {
  if (!url) return null;
  // Handle case where url is an object (common backend mismatch)
  const actualUrl = typeof url === "object" ? url.url || url.src : url;
  if (!actualUrl) return null;

  if (actualUrl.startsWith("http") || actualUrl.startsWith("blob:"))
    return actualUrl;
  const cleanPath = actualUrl.startsWith("/") ? actualUrl.slice(1) : actualUrl;
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const loadCurrentUserAndProfile = async () => {
      try {
        ...
        // Handle API returning user info inside posts response
        const rawPostsArray = Array.isArray(postsResponse)
          ? postsResponse
          : postsResponse?.posts || [];

        // 4. PREPARE PROFILE DATA
        // (Fix: Ensure we have fallback profile data if post.user is just an ID)
        // If we are viewing "Me", use meData. If viewing "Other", we might need to rely on what we have.
        const fallbackProfile =
          meData?._id === profileUserId
            ? meData
            : { ...meData, _id: profileUserId };

        // If the API returns the user object at root, use it (Common pattern)
        const apiUserProfile =
          postsResponse?.user ||
          (meData?._id === profileUserId ? meData : null);
        ...
        const normalizedPosts = rawPostsArray.map((post) => {
          ...
          // --- FIX: ROBUST NAME/AUTHOR RESOLUTION ---
          let postUser = post.user || post.author || {};

          // If postUser is just a string ID (Backend mismatch), use apiUserProfile or fallback
          if (typeof postUser === "string") {
            if (
              apiUserProfile &&
              (apiUserProfile._id === postUser ||
                apiUserProfile.id === postUser)
            ) {
              postUser = apiUserProfile;
            } else if (fallbackProfile._id === postUser) {
              postUser = fallbackProfile;
            } else {
              // Last resort: keep it as object with just ID
              postUser = { _id: postUser, username: "User" };
            }
          }
          ...
          const realAvatar = postUser.profileImage || postUser.avatar;
        ...
        setProfile(apiUserProfile || fallbackProfile);
  ...
  // --- TOGGLE LIKE FUNCTION (unchanged) ---
  ...
  // ... (Other handlers unchanged: addComment, removeComment, etc.) ...
  const addComment = async (postId, text) => {
    if (!text.trim()) return;
    try {
      const response = await commentOnPost(postId, text);
      const commentData =
        response.post?.comments?.slice(-1)[0] || response.comment || response;
      const newComment = {
        id: commentData._id,
        ...
        author: {
          ...
          name: commentData.user?.name || currentUser?.name || "You",
  ...
  const submitComposer = async (payload) => {
    ...
    try {
      ...
      // Normalize new post immediately
      const gallery = [];
      if (postData.images)
  ...
  return (
    <div className="profile-page">
      ...
      <div className="profile-two-column">
        ...
        {/* --- FIX IS HERE: Added zIndex and Relative position --- */}
        <section
          className="post-feed"

...
// --- CONFIG ---
// Fallback for base URL if not provided by env
...
const ensureAbsoluteUrl = (url) => {
  if (!url) return null;
  // If it's already absolute (http/blob), return it
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const loadCurrentUserAndProfile = async () => {
      try {
        ...
        // 4. NORMALIZE DATA (The Fix)
        const normalizedPosts = rawPostsArray.map((post) => {
          // --- FIX 1: BUILD MEDIA GALLERY FOR POSTCARD ---
          const gallery = [];
          ...
          // --- FIX 2: STRICT NAME PRIORITY ---
          // Backend sends: post.user = { name: "Farhan Uddin", username: "farhan" }
          // We MUST pick 'name' first.
  ...
  const toggleLike = async (postId) => {
    ...
    const willLike = !post.liked; // Calculate new state

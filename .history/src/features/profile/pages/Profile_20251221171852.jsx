...
export default function ProfilePage() {
    ...
    const loadCurrentUserAndProfile = async () => {
      try {
        ...
        const normalizedPosts = rawPostsArray.map((post) => {
          // --- FIX 1: Explicit Media Gallery Construction ---
          const gallery = [];
          ...
          // --- FIX 2: Explicit Name Priority ---
          // We check `name` first. If it exists, we use it.
          const realName =
  ...
  const submitComposer = async (payload) => {
    ...
    try {
      ...
      // Local helper re-use or copy logic
      const gallery = [];
      // (Repeat logic or use a function - simpler to just rebuild for single post)
      // For new posts, backend usually returns absolute URLs or we have local preview
      // But let's stick to standard flow:

      const newPost = {
        ...postData,
        id: postData._id,
        // Simple local construction for immediate feedback
        mediaGallery: [], // Will populate below
        author: {
          id: currentUser._id,
          name: currentUser.name || currentUser.username || "You",
          avatar: currentUser.profileImage
      ...
      if (postData.images)
        postData.images.forEach((img) =>
          newPost.mediaGallery.push({
            type: "image",
            src: ensureAbsoluteUrl(img),
          })
        );
      if (postData.videos)
        postData.videos.forEach((vid) =>
          newPost.mediaGallery.push({
            type: "video",
            src: ensureAbsoluteUrl(vid),
          })
        );
      newPost.media = newPost.mediaGallery[0] || null;

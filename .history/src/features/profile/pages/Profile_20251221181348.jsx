...
const ensureAbsoluteUrl = (url) => {
  if (!url) return null;
  // Fix: Handle case where url might be an object inside the array
  ...
  useEffect(() => {
    const loadCurrentUserAndProfile = async () => {
      try {
        ...
        // 1. PREPARE FALLBACK PROFILE (Fix for missing names)
        // If we are on a user's profile, and the post doesn't have user info, use this profile.
        let loadedProfile =
          meData?._id === profileUserId
            ? meData
            : { ...meData, _id: profileUserId };

        // If the API returned the user object alongside posts, use that
        if (postsResponse?.user) {
          loadedProfile = postsResponse.user;
        }
        ...
        const normalizedPosts = rawPostsArray.map((post) => {
          ...
          // --- FIX: USER NAME / AUTHOR LOGIC ---
          // Sometimes backend sends post.user as a STRING ID. We must handle that.
          let postUser = post.user || post.author || {};

          if (typeof postUser === "string") {
            // If it's just an ID, try to match it with the profile we just loaded
            if (
  ...
  return (
    <div className="profile-page">
      ...
      <div className="profile-two-column">
        ...
        {/* --- MAJOR FIX: Added z-index to bring feed to front --- */}
        <section
          className="post-feed"

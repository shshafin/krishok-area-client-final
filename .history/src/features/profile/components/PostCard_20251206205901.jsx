...
const mediaContainerStyles = {
  width: "100%",
  height: "360px", // Slightly increased for better aspect ratio
  display: "grid",
  gap: "2px",
  marginTop: "12px",
  position: "relative",
  overflow: "hidden",
  borderRadius: "8px",
  backgroundColor: "#f0f2f5", // Changed from black to neutral gray to avoid visual gaps
  // Centering safeguards
  marginLeft: "auto",
  marginRight: "auto",
};
...
export default function PostCard({
  ...
  // --- 1. FIXED GRID CONFIGURATION (BALANCED) ---
  const getGridConfig = (count) => {
    ...
    // 3 Images: 50/50 Split (Left Big, Right Stacked)
    // CHANGED: From "2fr 1fr" to "1fr 1fr" to center the balance
    if (count === 3) {
      return {
        gridTemplateColumns: "1fr 1fr",
    ...
    // 4+ Images: 50/50 Split
    // CHANGED: From "2fr 1fr" to "1fr 1fr" to center the balance
    if (count >= 4) {
      return {
        gridTemplateColumns: "1fr 1fr",
        gridTemplateRows: "1fr 1fr 1fr", // Right side splits into 3 rows
      };
    }
    return {};
  };
  ...
  const getItemStyle = (index, count) => {
    const baseStyle = {
      width: "100%",
      height: "100%",
      objectFit: "cover", // Forces image to fill the cell (no black bars)
      objectPosition: "center", // Ensures the image is centered in the cell
      display: "block",
    };

    // Logic for spanning the first image (The Big Left Image)
    if (count === 3 && index === 0) {
      return { ...baseStyle, gridRow: "span 2" };
    }
    if (count >= 4 && index === 0) {
      // Span 3 rows to match the 3 small images on right

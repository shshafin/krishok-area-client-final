...
export default function PostComposerModal({
  ...
  useEffect(() => {
    ...
    const autoOpenTimeout = setTimeout(() => {
      if (!readOnly && mode !== "text" && fileInputRef.current) {
  ...
  const handleSubmit = async () => {
    if (readOnly || (!text.trim() && attachments.length === 0)) return;
  ...
  const disableSubmit =
    readOnly || submitting || (!text.trim() && attachments.length === 0);
        ...
        {!readOnly && (
          <label className="composer-upload">
            ...
            <p style={{ marginTop: "0.35rem", color: "#64748b" }}>
              সমর্থিত ফরম্যাট: JPG, PNG, GIF, MP4, WEBM (সর্বোচ্চ{" "}
              {maxAttachments} ফাইল)
            </p>
            <input
              name="attachments"

...
// --- ALIGNMENT STYLES (From your preferred code) ---
// We use these specifically for the images to get that perfect alignment
const mediaContainerStyles = {
  width: "100%",
  height: "400px", // Fixed height ensures the "good alignment" you wanted
  display: "grid",
  gap: "2px",
  cursor: "pointer",
  position: "relative",
  overflow: "hidden",
  marginTop: "12px",
  // We don't set bg color here so it matches your theme's bg
};
...
export default function PostCard({
  ...
  // --- CHANGED: EXACT ASSET LOGIC FROM YOUR SNIPPET ---
  ...
  // 1. Grid Configuration (The Layout Logic)
  const getGridConfig = (count) => {
    if (count === 1) {
      // For 1 item, we allow auto height so it doesn't crop awkwardly
      return {
  ...
  // 2. Item Spanning (The "Collage" Logic)
  const getItemStyle = (index, count) => {
    const baseStyle = {
      width: "100%",
      height: "100%",
      objectFit: "cover", // Ensures images fill the grid cells perfectly
      display: "block",
    };
  ...
  return (
    // We use your original className "post-card" to keep the background/colors
    <article className="post-card">
      ...
      {/* --- MEDIA GALLERY (Using the Grid Logic) --- */}
      {totalItems > 0 && (
        <div
          className="post-media"
          {...mediaInteractableProps}
          // Applying the logic that fixes alignment
          style={{

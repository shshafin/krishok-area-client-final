...
export default function PostCard({
  ...
  // --- CHANGED: UNIFIED GALLERY LOGIC ---
  // 1. Get ALL items (Video + Images) into one flat array
  ...
  // We show a max of 4 items in the grid
  ...
  return (
    <article className="post-card">
      ...
      {/* --- CHANGED: SINGLE UNIFIED MEDIA BLOCK --- */}
      {totalItems > 0 && (
        <div
          className="post-media post-media--has-gallery"
          data-count={totalItems}
          {...mediaInteractableProps}>
          {/* If we have 1 item: Show Single View
             If we have 2+ items: Show Grid View
          */}
          {totalItems === 1 ? (
            // SINGLE ITEM LAYOUT
            ...
            // GRID LAYOUT (2, 3, or 4 items)
            // Ensure your CSS has classes like .count-2, .count-3 etc.
            <div className={`post-media-grid count-${displayItems.length}`}>
              {displayItems.map((item, index) => {
                const isOverflowItem = extraImageCount > 0 && index === 3; // Last item of 4
                ...
                return (
                  <div
                    ...
                    {item.type === "video" ? (
                      <video
                        // If it's the first item, attach the ref for visibility hook

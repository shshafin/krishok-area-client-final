...
export default function PostCard({
  ...
  const { normalizedGallery, displayAuthor } = useMemo(() => {
    ...
    // 2. Author (Trust Normalized Data first)
    const userObj = post.user || {};
    const authObj = post.author || {};
    // PRIORITY: post.author.name (from Parent) > post.user.name (Raw) > Username
    const realName =
      authObj.name ||
      userObj.name ||
      authObj.username ||
    ...
    const avatarRaw =
      authObj.avatar || authObj.profileImage || userObj.profileImage;
  ...
  const handleLike = (e) => {
    e.preventDefault();
    e.stopPropagation();
    if (onLike) onLike(POST_ID);
  };

  const handleCommentClick = (e) => {
    e.preventDefault();
    e.stopPropagation();
    if (onOpenComments) onOpenComments(POST_ID);
  };
  ...
  return (
    <article
      ...
      {/* ACTION BUTTONS: FIXED Z-INDEX & EVENTS */}
      <div

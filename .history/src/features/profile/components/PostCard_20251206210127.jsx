...
const mediaContainerStyles = {
  ...
  display: "grid",
  gap: "1px", // Reduced gap to 1px to "remove space"
  marginTop: "12px",
  position: "relative",
  overflow: "hidden",
  borderRadius: "8px",
  // Removed black background to prevent "black bar" look
...
export default function PostCard({
  ...
  // --- 1. REVERTED GRID CONFIG (Your preferred "Big Left" layout) ---
  const getGridConfig = (count) => {
    ...
    // 3 Images: LEFT SIDE BIG (2fr), RIGHT SIDE SMALL (1fr)
    if (count === 3)
      return {
    ...
    // 4+ Images: LEFT SIDE BIG (2fr), RIGHT SIDE SMALL (1fr)
    if (count >= 4)
      return {
        gridTemplateColumns: "2fr 1fr",
  ...
  // --- 2. FORCED FILL (Removes the spaces/black bars) ---
  const getItemStyle = (index, count) => {
    const baseStyle = {
      width: "100%",
      height: "100%",
      objectFit: "cover", // CRITICAL: Forces image to fill the void
      objectPosition: "center", // Keeps subject centered
      display: "block",
    };

    // Logic for spanning the first image (Big Left Image)
    if (count === 3 && index === 0) {

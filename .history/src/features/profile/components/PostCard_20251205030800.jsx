...
export default function PostCard({
  ...
  // --- FIXED LOGIC START ---
  const media = post.media; // The Main Item (Video or 1st Image)

  // 1. Get ALL items (both video and image)
  const allGalleryItems = Array.isArray(post.mediaGallery)
    ? post.mediaGallery.filter((item) => item && item.src)
    : [];

  // 2. Determine what to show in the "Grid" below the main item
  // Usually, we show everything EXCEPT the main item (so it doesn't repeat)
  // BUT if there is ONLY 1 item total, the grid should be empty.
  let galleryGridItems = [];

  if (allGalleryItems.length > 1) {
    // If we have multiple items, skip the first one (because it's already shown as 'media' above)
    galleryGridItems = allGalleryItems.slice(1);
  } else {
    // If only 1 item total, it's shown as 'media', so grid is empty
    galleryGridItems = [];
  }

  const totalGalleryImages = galleryGridItems.length;
  // Show max 4 items in the grid preview
  const extraImageCount = totalGalleryImages > 4 ? totalGalleryImages - 4 : 0;
  const visibleGallery = extraImageCount
    ? galleryGridItems.slice(0, 4)
    : galleryGridItems;
  ...
  return (
    <article className="post-card">
      ...
      {/* 1. RENDER MAIN MEDIA (Video or Image) */}
      {media && (
        <div
      ...
      {/* 2. RENDER GALLERY GRID (Remaining items) */}
            ...
            {visibleGallery.map((item, index) => {
              ...
              return (
                <div
                  ...
                  {/* Handle Video Thumbnails in Grid */}
                  {item.type === "video" ? (
                    <video
                      src={item.src}
                      style={{
                  ...
                  {/* Optional: Add Play Icon overlay if video */}
...
PostCard.propTypes = {
  post: PropTypes.shape({
    ...
    mediaGallery: PropTypes.arrayOf(
      PropTypes.shape({
        type: PropTypes.oneOf(["image", "video"]), // Allow video here too
        src: PropTypes.string,
      })
    ),

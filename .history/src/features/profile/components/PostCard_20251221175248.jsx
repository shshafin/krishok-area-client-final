...
export default function PostCard({
  ...
  const { normalizedGallery, displayAuthor } = useMemo(() => {
    // 1. Build Gallery (Auto-detect missing mediaGallery)
    ...
    // 2. Resolve Author Name (THE NAME FIX)
    const userObj = post.user || {}; // Raw Backend Data (Contains "Farhan Uddin")
    const authObj = post.author || {}; // Frontend Data (Might contain "farhan")

    // PRIORITY: Check Raw Backend Name first!
    const realName =
      userObj.name ||
      authObj.name ||
      userObj.username ||
      authObj.username ||
      "Unknown";
    const userId = userObj._id || userObj.id || authObj._id || authObj.id;
  ...
  return (
    <article
      ...
      {/* ACTION BUTTONS (LIKE/COMMENT) - FIXED CLICKABILITY */}
      <div

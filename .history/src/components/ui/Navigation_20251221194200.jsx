...
import { fetchNotifications } from "@/api/authApi";
...
function Navigation() {
  ...
  // NEW: Handle clicking a notification to navigate
  const handleNotificationClick = (notification) => {
    // 1. Close the popover
    setIsOpen(false);

    // 2. Extract Post ID
    // Check if post is populated (object) or just an ID (string)
    const postId =
                ...
                notifications.map((notification) => (
                  <article
                    ...
                    <button
                      ...
                      // Stop propagation prevents the article onClick from firing when clicking X
                      onClick={(e) => {
                        e.stopPropagation();

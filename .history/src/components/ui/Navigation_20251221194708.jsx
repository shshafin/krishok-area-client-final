...
import {
  fetchNotifications,
  // Assuming these exist or you will add them to authApi
  markNotificationRead,
  clearAllNotifications,
} from "@/api/authApi";
import { baseApi } from "../../api"; // Import baseApi to fix image paths
...
function Navigation() {
  ...
  const handleDismiss = async (id) => {
    ...
    // 2. API Call (To persist deletion)
    try {
  ...
  // Determine unread count (assuming API has isRead field, defaulting to length if not)
  ...
  return (
    <nav
            ...
            <div
              ...
              {notifications.length > 0 ? (
                notifications.map((notification) => {
                  ...
                  const name =
                    sender.name || sender.username || "অজানা ব্যবহারকারী"; // Fixed Name
                  const avatar = sender.profileImage
                    ? ensureAbsoluteUrl(sender.profileImage)
                    : AVATAR_PLACEHOLDER; // Fixed Image
      ...
      {/* --- MOBILE STYLES (Inline for Quick Fix) --- */}
      <style>{`
        /* Desktop Default Styles are assumed to exist in your CSS */
        ...
        @media (max-width: 768px) {
          .notification-popover.mobile-full-screen {
            ...
            transform: none !important; /* Override any popper transforms */
          }

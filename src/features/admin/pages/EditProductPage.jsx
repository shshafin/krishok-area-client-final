    ...
    const loadProduct = async () => {
      try {
        ...
        if (res?.success) {
          ...
          setBenefits(
            Array.isArray(data.beboharerShubidha)
              ? data.beboharerShubidha.join("\n")
          ...
          try {
            crops = Array.isArray(data.foshol)
              ? data.foshol
              : JSON.parse(data.foshol || "[]");
            pests = Array.isArray(data.balai)
              ? data.balai
              : JSON.parse(data.balai || "[]");
            dosages = Array.isArray(data.matra)
              ? data.matra
              : JSON.parse(data.matra || "[]");
            instructions = Array.isArray(data.beboharBidhi)
              ? data.beboharBidhi
              : JSON.parse(data.beboharBidhi || "[]");
          } catch (err) {
            console.error("Failed to parse application arrays", err);
          }
